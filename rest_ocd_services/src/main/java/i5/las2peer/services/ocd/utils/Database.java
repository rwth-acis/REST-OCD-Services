...
public class Database {
	...
	public void createCollections() {
		...
		collectionNames.add(MultiplexCommunity.multiplexCollectionName); // 13
		collection = db.collection(MultiplexCommunity.multiplexCollectionName);
		if (!collection.exists()) {
			collection.create();
		}
	...
	/**
	 * Updates a persisted graph by updating Attributes,nodes,edges and
	 * creationMethod
	 * does NOT update changes in the covers or CentralityMaps that run on the given
	 * graph
	 * 
	 * @param graph
	 *              the graph
	 ...
	 * @param key
	 *                 key of the graph
	 * @return the found CustomGraph instance or null if the CustomGraph does not
	 *         exists or the username is wrong
	 */
	public CustomGraph getGraph(String username, String key) {
		...
		if (g == null) {
			...
			logger.log(Level.WARNING,
					"user: " + username + " is not allowed to use Graph: " + key + " with user: " + g.getUserName());
			g = null;
		}
	...
	public List<CustomGraph> getGraphs(String username) {
		...
		try {
			...
			String queryStr = "FOR g IN " + CustomGraph.collectionName + " FILTER g." + CustomGraph.userColumnName
					+ " == @username RETURN g._key";
	...
	/**
	 * Return specified graphs' meta information of a user using an efficient
	 * approach. This approach only necessary
	 * metadata about graphs. E.g. no information about nodes/edges (other than
	 * their count) is loaded.
	 *
	 * @param username
	 *                           the users username
	 * @param firstIndex
	...
	public ArrayList<CustomGraphMeta> getGraphMetaDataEfficiently(String username, int firstIndex, int length,
		...
		try {
			AqlQueryOptions queryOpt = new AqlQueryOptions().streamTransactionId(transId);
			String queryStr = "FOR g IN " + CustomGraph.collectionName + " FOR gcl IN "
					+ GraphCreationLog.collectionName +
					" FILTER g." + CustomGraph.userColumnName + " == @username AND gcl._key == g."
					+ CustomGraph.creationMethodKeyColumnName +
			...
			/*
			 * Create CustomGraphMeta instances based on the queried results and add them to
			 * the list to return
			 */
	...
	public List<CustomGraph> getGraphsbyName(String name) {
		...
		try {
			AqlQueryOptions queryOpt = new AqlQueryOptions().streamTransactionId(transId);
			String queryStr = "FOR g IN " + CustomGraph.collectionName + " FILTER g." + CustomGraph.nameColumnName
					+ " == @name RETURN g._key";
	...
	private void deleteGraph(String key) {
		...
		try {
			...
			///////////// THIS PART SHOULD NOT BE NEEDED BUT ASSURES NO COVER OR CENTRALITY
			///////////// MAP IS MISSED/////////////
	...
	public Cover getCover(String username, String graphKey, String coverKey) {
		...
		if (cover == null) {
			logger.log(Level.WARNING,
					"user: " + username + ", " + "Cover does not exist: cover id " + coverKey + ", graph id "
							+ graphKey);
		}
		return cover;
	}
	...
	public List<Cover> getCoversByName(String name, CustomGraph g) {
		...
		try {
			AqlQueryOptions queryOpt = new AqlQueryOptions().streamTransactionId(transId);
			String queryStr = "FOR c IN " + Cover.collectionName + " FILTER c." + Cover.nameColumnName
					+ " == @name RETURN c._key";
			Map<String, Object> bindVars = Collections.singletonMap("name", name);
	...
	public List<Cover> getCovers(String username, String graphKey) { // TODO testen
		...
		try {
			AqlQueryOptions queryOpt = new AqlQueryOptions().streamTransactionId(transId);
			String queryStr = "FOR c IN " + Cover.collectionName + " FILTER c." + Cover.graphKeyColumnName
					+ " == @key RETURN c._key";
	...
	public List<Cover> getCovers(String username, String graphKey, List<Integer> executionStatusIds,
		...
		try {
			...
			String queryStr = " FOR c IN " + Cover.collectionName + " FOR a IN " + CoverCreationLog.collectionName +
					" FILTER c." + Cover.creationMethodKeyColumnName + " == a._key AND a."
			...
			if (metricExecutionStatusIds != null && metricExecutionStatusIds.size() > 0) {
				queryStr += " FOR m IN " + OcdMetricLog.collectionName + " FILTER m." + OcdMetricLog.coverKeyColumnName
						+ " == c._key AND " + "m." +
			...
			if (!graphKey.equals("")) { // es gibt einen graphKey
				...
				queryStr += " FOR g IN " + CustomGraph.collectionName +
						" FILTER g." + CustomGraph.userColumnName + " == @user AND c." + Cover.graphKeyColumnName
						+ " == g._key";
	 ...
	 * @return a cover list
	 */
	public List<CoverMeta> getCoverMetaDataEfficiently(String username, String graphKey,
			List<Integer> executionStatusIds,
			List<Integer> metricExecutionStatusIds, int firstIndex, int length) {
		String transId = getTransactionId(null, false);
		AqlQueryOptions queryOpt = new AqlQueryOptions().streamTransactionId(transId);
		...
		try {
			...
			// if (metricExecutionStatusIds != null && metricExecutionStatusIds.size() > 0)
			// {
			// queryStr += " FOR m IN " + OcdMetricLog.collectionName + " FILTER m." +
			// OcdMetricLog.coverKeyColumnName + " == c._key AND " +"m." +
			// OcdMetricLog.statusIdColumnName + " IN " + metricExecutionStatusIds;
			// }
			if (!graphKey.equals("")) { // es gibt einen graphKey
				queryStr += " FOR g IN " + CustomGraph.collectionName
	...
	private void deleteCover(String key, String transId) { // Always use it inside a transaction
		...
		Boolean isMultiplex = Boolean.parseBoolean(coverDoc.getAttribute(Cover.is_Multiplex).toString());
		for (String communityKey : communityKeys) { // delete all communitys
			if (isMultiplex) {
				ArangoCollection multiplexCommunityCollection = db.collection(MultiplexCommunity.multiplexCollectionName);
				multiplexCommunityCollection.deleteDocument(communityKey, null, deleteOpt);
			} else {
	 ...
	 */
	public void deleteCover(String username, String graphKey, String coverKey, ThreadHandler threadHandler)
			throws Exception {
	...
	public CentralityMap getCentralityMap(String username, String graphKey, String mapKey) {
		...
		if (map == null) {
			logger.log(Level.WARNING,
					"user: " + username + ", " + "CentralityMap does not exist: map key " + mapKey + ", graph key "
							+ graphKey);
		}
		return map;
	}
	...
	public List<CentralityMap> getCentralityMaps(String username, String graphKey) {
		...
		try {
			AqlQueryOptions queryOpt = new AqlQueryOptions().streamTransactionId(transId);
			String queryStr = "FOR c IN " + CentralityMap.collectionName + " FILTER c."
					+ CentralityMap.graphKeyColumnName + " == @key RETURN c._key";
			Map<String, Object> bindVars = Collections.singletonMap("key", graphKey);
	 ...
	 */
	public List<CentralityMap> getCentralityMaps(String username, String graphKey, List<Integer> executionStatusIds,
			int firstIndex, int length) { // TODO testen
		String transId = getTransactionId(null, false);
		AqlQueryOptions queryOpt = new AqlQueryOptions().streamTransactionId(transId);
		DocumentReadOptions readOpt = new DocumentReadOptions().streamTransactionId(transId);
		...
		try {
			String queryStr = " FOR c IN " + CentralityMap.collectionName + " FOR a IN "
					...
					" FILTER g." + CustomGraph.userColumnName + " == @user AND c." + CentralityMap.graphKeyColumnName
					+ " == g._key AND c." +
	 ...
	 */
	public List<CentralityMeta> getCentralityMapsEfficiently(String username, String graphKey,
			List<Integer> executionStatusIds, int firstIndex, int length) { // TODO testen
		String transId = getTransactionId(null, false);
		AqlQueryOptions queryOpt = new AqlQueryOptions().streamTransactionId(transId);
		HashMap<String, Object> bindVars = new HashMap<String, Object>();
		bindVars.put("user", username);
	 ...
	 *            the centralityMap
	 */
	public void updateCentralityMap(CentralityMap map) { // existenz der map muss bereits herausgefunden worden sein
															// TESTEN
		String transId = this.getTransactionId(CentralityMap.class, true);
		try {
	 ...
	 */
	public void deleteCentralityMap(String username, String graphKey, String mapKey, ThreadHandler threadHandler)
			throws Exception {
	 ...
	 * @return the found OcdMetricLog instance or null if the Cover or Graph does
	 *         not exist
	 */
	public OcdMetricLog getOcdMetricLog(String username, String graphKey, String coverKey, String metricKey) {
		...
		if (metric == null) {
			logger.log(Level.WARNING,
					"user: " + username + ", " + "OcdMetricLog does not exist: cover id " + coverKey + ", graph id "
							+ graphKey + ", metric id " + metricKey);
		}
		return metric;
	}
	 ...
	 * @param firstIndex   Index of the first simulation to return (only relevant if
	 *                     return Subset is true)
	 * @param length       Number of simulations (only relevant if returnSubset is
	 *                     true)
	 * @param graphKey     The graph's key
	 * @return List of SimulationSeries of a specific user
	 */
	public List<SimulationSeries> getSimulationSeriesByUser(String userId, Boolean returnSubset, int firstIndex,
			int length, String graphKey) {
		...
		try {
			AqlQueryOptions queryOpt = new AqlQueryOptions().streamTransactionId(transId);
			String queryStr = "FOR ss IN " + SimulationSeries.collectionName + " FILTER ss."
					+ SimulationSeries.userIdColumnName + " == @username ";
			if (graphKey != "") {
				queryStr += " AND ss." + SimulationSeries.simulationSeriesParametersColumnName + ".graphKey == @gKey "; // graphKey
																														// refers
																														// to
																														// graphKey
																														// field
	...
	private void deleteSimulationSeries(String key, String transId) {
		...
		/*
		 * Delete simulation series groups of which the deleted simulation series was a
		 * part of
		 */
		String query = "FOR ssg IN " + SimulationSeriesGroup.collectionName + " FILTER \"" + key + "\" IN ssg."
				+ SimulationSeriesGroup.simulationSeriesKeysColumnName
				+ " RETURN ssg._key";
	 ...
	 */
	public List<SimulationSeries> getSimulationSeriesByUser(String userId, String graphKey, int firstIndex,
			int length) {
	...
	public List<SimulationSeriesGroup> getSimulationSeriesGroups(String userId, Boolean returnSubset, int firstIndex,
			int length) {
		String transId = getTransactionId(SimulationSeriesGroup.class, false);
	...
	/**
	 * Removes username from the InactivityData table from the database. This is to
	 * ensure that for users whose content
	 * has been deleted and who are inactive, no processing power is wasted for
	 * checking their data.
	 *
	...
	public void deleteUserInactivityData(String username, ThreadHandler threadHandler) {
		...
		synchronized (threadHandler) {
			try {
				...
				String query = "FOR d IN " + InactivityData.collectionName + " FILTER d."
						+ InactivityData.userColumnName +
	...
	public String getTransactionId(Class c, boolean write) {
		...
		if (c == CustomGraph.class) {
			...
			List<String> collectionList = new ArrayList(collectionNames.subList(4, 8));
			collectionList.add(collectionNames.get(13));
			collections = collectionList.toArray(new String[5]);
