package i5.las2peer.services.ocd.spmf;
...
public class AlgoAprioriClose {
	 ...
	 * @param transactions  a map where each key is a transaction id and each value is a list of item ids
	 */
	public Itemsets runAlgorithm(double minsup, Map<Integer, List<Integer>> transactions) {
		...
		for (Map.Entry<Integer, List<Integer>> entry : transactions.entrySet()) {
			List<Integer> values = entry.getValue();

			// create an array of int to store the items in this transaction
			int transaction[] = new int[values.size()];
...
//				saveItemsetToFile(entry.getKey(), entry.getValue());
	...
	private void checkIfItemsetsK_1AreClosed(List<Integer> levelKm1,
			List<Itemset> levelK, Map<Integer, Integer> mapItemCount) {
		// for each itemset of size k-1
...
void saveItemset(Itemset itemset) {
	 ...
	 void saveItemset(Integer item, Integer support){
		itemsetCount++;
