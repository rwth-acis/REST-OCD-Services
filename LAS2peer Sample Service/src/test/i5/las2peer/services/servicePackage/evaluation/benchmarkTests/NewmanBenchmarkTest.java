...
import i5.las2peer.services.servicePackage.benchmarkModels.NewmanModel;
		...
		for(int i=0; i<9; i++) {
			...
			for(int j=0; j<10; j++) {
				...
				for(int k=0; k<algos.length; k++) {
					...
					timeAverages[k] += cover.getMetric(MetricType.EXECUTION_TIME).getValue() / 10d;
					nmiAverages[k] += cover.getMetric(MetricType.EXTENDED_NORMALIZED_MUTUAL_INFORMATION).getValue() / 10d;
					omegaAverages[k] += cover.getMetric(MetricType.OMEGA_INDEX).getValue() / 10d;
				}
			}
