package i5.las2peer.services.servicePackage.evaluation.benchmarkTests;
...
import i5.las2peer.services.servicePackage.evaluation.BenchmarkAveragesOutputAdapter;
 ...
 */
@Ignore
public class LfrBenchmarkMu01K24Test {
	...
	public void testOnNewman() throws IOException, OcdAlgorithmException, AdapterException, MetricException, BenchmarkException
	{
		...
		for(int i=0; i<11; i++) {
			...
			for(int j=0; j<5; j++) {
				GroundTruthBenchmarkModel model = new LfrModel(24, 0.1, i*0.1);
				...
				GraphOutputAdapter graphAdapter = new WeightedEdgeListGraphOutputAdapter(
						new FileWriter(EvaluationConstants.lfrGraphOutputPath + "k24mu01" + "param" + i + "it" + j + ".txt"));
				graphAdapter.writeGraph(graph);
				CoverOutputAdapter groundTruthAdapter = new EvaluationLabeledMembershipMatrixOutputAdapter(
						new FileWriter(EvaluationConstants.lfrCoverOutputPath + "k24mu01" + "param" + i + "it" + j + "groundTruth" + ".txt"));
				groundTruthAdapter.writeCover(groundTruth);
				for(int k=0; k<algos.length-1; k++) {
					...
					CoverOutputAdapter coverAdapter = new EvaluationLabeledMembershipMatrixOutputAdapter(
							new FileWriter(EvaluationConstants.lfrCoverOutputPath + "k24mu01" + "param" + i + "it" + j + algoFileNameExtensions[k]));
			...
			BenchmarkAveragesOutputAdapter metricAdapter = new BenchmarkAveragesOutputAdapter(
					new FileWriter(EvaluationConstants.lfrMetricsOutputPath + "k24mu01" + "param" + i + ".txt"));
